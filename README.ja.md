...
リポジトリには将来の作例用に [`assets/examples/`](assets/examples/) を用意しています。プロジェクト作者が確認した 012 の完成作品だけを追加し、それまでは別スタイルの投稿や画像を代用しません。

将来の作例は 012 の応用範囲を示すだけで、主体、比喩、配色、文言、画角が生成参照や既定値になることはありません。

## 4つの出力を支えるひとつの生命色線場ロジック
...
| モード | サイズ方針 | 成果物 |
| --- | --- | --- |
| `top-bottom` | 元画像適応 | 上に完全な元写真、下に 012 の生命色幾何線場。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `left-right` | 元画像適応 | 左に完全な元写真、右に 012 の生命色幾何線場。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `design-only` | 元画像適応 | 元写真を見せず、変化デザインだけを表示。元画像の比率と寸法を維持 |
| `wallpaper-pack` | 端末別4サイズ | スマートフォン、iPad、デスクトップ、子ども用ウォッチの PNG を個別出力 |

優先順位は、正確な指定ピクセル > 指定比率／用途 > 通常モードの元画像適応です。原稿 `012.md` の 3:4 は初期の制作画角であり、現在の暗黙の既定値ではありません。
...
- **独立4作品：** 各端末は元写真だけを参照し、密度中心、線場方向、幾何骨格、生命色の置き方、文字関係を別々に探れます。
...
画像モデルが元写真に結びついた集積主体、高密度の反復線、ずれと折り返し、外周への希薄化、幾何学的制御、一つの生命色、黒灰の小文字を作ります。`scripts/compose_panel.py` は画布計画、厳密な 50/50 ラスター合成、最終寸法、監査だけを担当し、プログラム描画で作品を偽装しません。
